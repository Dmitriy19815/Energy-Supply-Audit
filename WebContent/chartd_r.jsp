...
//	String z3 = "FFEAEA";
	...
	for (int i=0; i<cc.DataDaySize(); i++) {
		if ( Math.abs(cc.getDataDayByNameDouble(i,"q_qua_0")) > max_qua ) {
			max_qua = Math.abs(cc.getDataDayByNameDouble(i,"q_qua_0"));
		}
		if ( Math.abs(cc.getDataDayByNameDouble(i,"q_gen_0")) > max_gen ) {
			max_gen = Math.abs(cc.getDataDayByNameDouble(i,"q_gen_0"));
		}
	}
	long scale_qua = 10;
	long scale_gen = 10;
	...
	try {
		...
		for (int i=0; i<49; i++) {
			for (int j=cc.DataDaySize()-1; j>=0; j--) {
				...
				if (Day==i) {
					...
					g.fillRect(sh_x+29+Day*14,260,13,200);
				}
			}
		}
		...
		for (int i=1; i <= step_qua; i++) {
			...
			g.drawString(str,sh_x+33-str.length()*7,Math.round(240+4-i*(200/step_qua)));
		}
        g.setColor(Color.BLACK);
		g.drawLine(sh_x+37,240,sh_x+42,240);

		for (int i=1; i <= step_gen; i++) {
			g.setColor(Color.LIGHT_GRAY);
			g.drawLine(sh_x+42,Math.round(260+i*(200/step_gen)),sh_x+42+48*14,Math.round(260+i*(200/step_gen)));
	        g.setColor(Color.BLACK);
			g.drawLine(sh_x+37,Math.round(260+i*(200/step_gen)),sh_x+42,Math.round(260+i*(200/step_gen)));
			str = Basic.formatNumber(1,10,0,0,scale_gen*i);
			g.drawString(str,sh_x+33-str.length()*7,Math.round(260+4+i*(200/step_gen)));
		}
        g.setColor(Color.BLACK);
		g.drawLine(sh_x+37,260,sh_x+42,260);
		...
		for (int i=0; i<49; i++) {
			for (int j=cc.DataDaySize()-1; j>=0; j--) {
				...
				if (Day==i) {
					...
					g.fillRect(sh_x+4+28+Day*14,240-(int)Math.round(Math.abs(cc.getDataDayByNameDouble(j,"q_qua_0"))*vpp_qua),6,(int)Math.round(Math.abs(cc.getDataDayByNameDouble(j,"q_qua_0"))*vpp_qua));
					g.setColor(new Color(Integer.parseInt(colr_gen,16)));
					g.fillRect(sh_x+4+28+Day*14,260,6,(int)Math.round(Math.abs(cc.getDataDayByNameDouble(j,"q_gen_0"))*vpp_gen));
				}
			}
		}
		...
		for (int i=0; i<48; i++) {
			if (parity) {
				...
				g.drawRect(sh_x+42+i*14,40,28,420);
			}
