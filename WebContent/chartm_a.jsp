...
<%
	...
	for (int i=1; i<cc.DataRangeSize(); i++) {
		if ( Math.abs(cc.getDataRangeByNameDouble(i,"p_qua_1")) > max_qua ) {
			max_qua = Math.abs(cc.getDataRangeByNameDouble(i,"p_qua_1"));
		}
		if ( Math.abs(cc.getDataRangeByNameDouble(i,"p_qua_2")) > max_qua ) {
			max_qua = Math.abs(cc.getDataRangeByNameDouble(i,"p_qua_2"));
		}
		if ( Math.abs(cc.getDataRangeByNameDouble(i,"p_qua_3")) > max_qua ) {
			max_qua = Math.abs(cc.getDataRangeByNameDouble(i,"p_qua_3"));
		}
	}
    ...
    try {
		...
		for (int i=1; i<cc.DataRangeSize(); i++) {
			...
			g.fillRect(sh_x+4+i*42+0*14,240-(int)Math.round(Math.abs(cc.getDataRangeByNameDouble(cc.DataRangeSize()-i,"p_qua_1"))*vpp_qua),6,(int)Math.round(Math.abs(cc.getDataRangeByNameDouble(cc.DataRangeSize()-i,"p_qua_1"))*vpp_qua));

			g.setColor(new Color(Integer.parseInt(colr_qua,16)));
			g.fillRect(sh_x+4+i*42+1*14,240-(int)Math.round(Math.abs(cc.getDataRangeByNameDouble(cc.DataRangeSize()-i,"p_qua_2"))*vpp_qua),6,(int)Math.round(Math.abs(cc.getDataRangeByNameDouble(cc.DataRangeSize()-i,"p_qua_2"))*vpp_qua));

			g.setColor(new Color(Integer.parseInt(colr_qua,16)));
			g.fillRect(sh_x+4+i*42+2*14,240-(int)Math.round(Math.abs(cc.getDataRangeByNameDouble(cc.DataRangeSize()-i,"p_qua_3"))*vpp_qua),6,(int)Math.round(Math.abs(cc.getDataRangeByNameDouble(cc.DataRangeSize()-i,"p_qua_3"))*vpp_qua));
...
//		g.setColor(Color.LIGHT_GRAY);
//		g.drawString(Basic.formatNumber(1,10,2,2,max_a),420,20);

        g.setColor(Color.BLACK);
		g.setFont(fontb);
		g.drawString("\u043A\u0412\u0442\u00B7\u0433",sh_x,17); //???*?
